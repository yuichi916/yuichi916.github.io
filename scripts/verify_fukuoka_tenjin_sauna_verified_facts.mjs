...
const row = rows[0];
if (row.length !== 23 || row[1] !== "テンジンサウナ" || row[5] !== "private_sauna" || row[10] !== 0 || row[14] !== "福岡市" || row[15] !== "08:00-23:30; irregular" || row[22] !== "2026-08-13") {
...
if (!entry.facts.every(fact => fact.official && fact.urls?.includes("https://www.tenjin-sauna.com/price/"))) {
  throw new Error("公式根拠URLが不正です。");
}
const expectedKeys = ["access", "hours", "price", "reservation", "solo_ok"];
if (entry.facts.map(fact => fact.k).sort().join(",") !== expectedKeys.join(",")) throw new Error("確認済み情報の種類が不正です。");
if (summary.total !== 40570 || summary.checked_count !== Object.keys(curated).length || summary.checked_count < 749) {
