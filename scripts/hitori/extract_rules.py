...
# 一人客がいちばん知りたいのが席なのに、40,615件中433件しか分かっていなかった。
# 書き方の幅（「カウンター(8席)」「カウンター：6名」「お席：カウンター8」）を拾う。
# 数字とカウンターの間に挟めるのは4文字まで、かつ数字を含まない（「カウンター席は
# 1階です」を1席にしないため）。単位は席・名・人に限る（「3分」「500円」を採らない）。
COUNTER_N = re.compile(r"カウンター(?:席)?[^。\n0-9]{0,4}(\d{1,3})\s*(?:席|名|人)")
COUNTER_Y = re.compile(r"(カウンター席|全席カウンター|カウンターのみ)")
# 店舗概要の表は単位を落とす（「席数 カウンター8 テーブル12」）。単位が無い数は
# 席の話だと分かる行——「席」の字がある行——でしか読まない。
COUNTER_BARE = re.compile(r"カウンター[:：\s]{0,2}(\d{1,3})(?![0-9階分円年月日時])")
# 「8月12日(水)臨時休業」のように日付・曜日が添えられた告知は、その日限りの話。
DATED = re.compile(r"(\d{1,2}\s*[月/]\s*\d{1,2}\s*[日）)]?|[（(][月火水木金土日][）)]|\d{4}\s*年|令和\d)")
SEATS_N = re.compile(r"(?:総?席数|座席数|客席数?|全席)[はが]?\s*[:：]?\s*(\d{1,3})\s*(?:席|名)(?!\s*円)")
...
def extract(text, url):
    ...
    for line in _lines(text):
        ...
        if "counter_seats" not in found:
            m = COUNTER_N.search(line) or ("席" in line and COUNTER_BARE.search(line))
            if m:
