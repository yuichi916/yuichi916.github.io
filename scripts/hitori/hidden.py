...
# 0.4はその約1.6倍チェーンが密集した場所を指す。以下は実測(全37,193件)で、
# 設定ごとに数字が変わるので設定名を必ず添えて読むこと。
#   0.6 / chains.py導入前            : 穴場  238件 (0.6%) — この過小評価が
#                                       ジャンカラを穴場に出し、調査の発端になった
#   0.6 / chains.py導入後            : 穴場  1.2%
#   0.4 / chains.py導入後・現在の設定 : 穴場 1,216件 (3.3%)
# 0.6は実データではなく想定分布に対して決めた値で、chains.py導入後でも
# 1.2%では画面に出す件数として少なすぎた。
