...
const row = rows[0];
if (row.length !== 23 || row[1] !== "ウェルビー福岡" || row[5] !== "sauna" || row[10] !== 1 || row[14] !== "福岡市" || row[15] !== "男性サウナ 5:00-23:00; 女性専用エリア 12:00-22:00" || row[22] !== "2026-08-13") {
...
const expectedKeys = ["conditions", "hours", "price", "reservation"];
if (entry.facts.map(fact => fact.k).sort().join(",") !== expectedKeys.join(",")) throw new Error("確認済み情報の種類が不正です。");
if (entry.facts.some(fact => fact.k === "solo_ok")) throw new Error("一人利用可否が未確認なのにsolo_okが登録されています。");
if (!entry.facts.every(fact => fact.official && fact.urls?.some(url => url === homeUrl || url === faqUrl))) throw new Error("公式根拠URLが不正です。");
if (!entry.facts.find(fact => fact.k === "conditions")?.v.includes("一人利用可否を直接確認できていない")) throw new Error("一人利用要確認の明記が不正です。");
if (!pageText.includes('conditions:"利用条件"')) throw new Error("利用条件ラベルの日本語表示が不正です。");
if (summary.total !== 40570 || summary.checked_count !== Object.keys(curated).length || summary.checked_count < 752) {
