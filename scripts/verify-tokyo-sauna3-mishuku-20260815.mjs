...
const id = "manual-tokyo-sauna3-mishuku-20260815";
...
if (!entry || entry.checked !== "2026-08-15" || entry.facts.length !== 8 || entry.sources.length !== 2) throw new Error("確認済み根拠、確認日、または情報源数が不正です。");
if (!entry.facts.some(fact => fact.k === "solo_ok" && fact.official && /1名様/.test(fact.v))) throw new Error("公式一人利用根拠が記録されていません。");
if (!entry.facts.some(fact => fact.k === "hours" && fact.official && /08:00/.test(fact.v))) throw new Error("公式営業時間が記録されていません。");
if (!entry.facts.some(fact => fact.k === "conditions" && fact.official && /同性同士/.test(fact.v))) throw new Error("公式利用条件が記録されていません。");
